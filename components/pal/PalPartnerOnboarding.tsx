...
export default function PalPartnerOnboarding({
  ...
  return (
    <div className="rounded-[1.5rem] border border-[rgba(13,15,18,0.1)] bg-white/90 dark:bg-white/85 backdrop-blur-sm p-8 mb-8 shadow-card-resting max-w-2xl mx-auto md:mx-0 text-[#0D0F12]">
      <div className="flex items-center justify-between gap-3 mb-2">
        ...
        {onDismiss ? (
          <button type="button" onClick={onDismiss} className="text-[12px] text-[#0D0F12]/60 hover:text-[#0D0F12] underline">
            Close
          </button>
        ) : null}
      </div>
      <p className="font-sans text-sm text-[#0D0F12]/75 mb-6 leading-relaxed">
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        ...
        <span className="inline-flex items-center gap-1.5 text-xs text-[#0D0F12]/65 self-center px-2">
      ...
      <div className="space-y-3 border-t border-[rgba(13,15,18,0.08)] pt-6">
        <label className="block text-[12px] font-medium text-[#0D0F12]/85">Partner&apos;s account ID</label>
        <input
          type="text"
          value={pasteId}
          onChange={(e) => setPasteId(e.target.value)}
          placeholder="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"
          className="w-full rounded-xl border border-[rgba(13,15,18,0.12)] px-4 py-3 text-sm font-mono text-[#0D0F12] placeholder:text-[#0D0F12]/40 outline-none focus:border-[var(--gold)]"
        />
        <label className="block text-[12px] font-medium text-[#0D0F12]/85">Nickname (optional)</label>
        <input
          type="text"
          value={nickname}
          onChange={(e) => setNickname(e.target.value)}
          placeholder="What you call them in the dashboard"
          className="w-full rounded-xl border border-[rgba(13,15,18,0.12)] px-4 py-3 text-sm text-[#0D0F12] placeholder:text-[#0D0F12]/40 outline-none focus:border-[var(--gold)]"
        />
        ...
        <button
          ...
          className="px-5 py-2.5 rounded-full bg-[var(--gold)] text-[#0D0F12] text-sm font-medium disabled:opacity-40 disabled:cursor-not-allowed"
        >
