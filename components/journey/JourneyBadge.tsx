...
export default function JourneyBadge({ onContinue, streakDays }: { onContinue: () => void; streakDays: number }) {
  ...
  return (
    <motion.div layoutId="panelContent" className={`${panelBaseClasses} items-center justify-center text-center`}>
      <motion.h2 initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="font-display font-bold text-3xl text-[#0D0F12] mb-12">
        Mashallah!
        <br />
        Gate Unlocked
      </motion.h2>
      <div className="relative mb-6">
        ...
        <motion.div
          ...
          className="w-[120px] h-[120px] rounded-full border-4 border-[var(--gold)] bg-[#F4EFE6] shadow-gold-glow flex items-center justify-center relative z-10"
        >
          <Sparkle weight="regular" size={48} className="text-[var(--gold)]" />
        </motion.div>
      </div>
      <h3 className="font-sans font-semibold text-lg text-[#0D0F12]/85 mb-8">Gate of Protection</h3>
      <div className="flex items-center gap-2 mb-12 bg-orange-50 px-4 py-2 rounded-full border border-orange-100">
        <motion.div animate={{ y: [-3, 0, -3] }} transition={{ repeat: Infinity, duration: 2, ease: "easeInOut" }}>
          <Fire weight="regular" size={16} className="text-orange-500" />
        </motion.div>
        <span className="font-sans font-medium text-sm text-orange-700">{streakLabel}</span>
      </div>
      <motion.button
        whileHover={{ scale: 0.98 }}
        whileTap={{ scale: 0.95 }}
        onClick={onContinue}
        className="w-full py-4 rounded-full bg-[var(--gold)] text-[#0D0F12] font-sans font-bold text-sm shadow-md mt-auto"
      >
        Continue Journey
      </motion.button>
    </motion.div>
